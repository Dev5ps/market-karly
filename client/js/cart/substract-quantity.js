...
export const substractQuantity = (pageIdNode, itemNode, buttonNode, localStorageKey) => {

  const refreshInfoForModifyQuantity = (productId, targetNode) => {
    ...
    targetNode.nextElementSibling.nextElementSibling.textContent = currentProductData.stock;
  ...
  const setProductDataToLocalStorageHandler = (e) => { 
    ...
    let substractItem;
    let current = JSON.parse(localStorage.getItem(localStorageKey))?.find(e => e.id === productId)

    if (JSON.parse(localStorage.getItem(localStorageKey))?.find(e => e.id === productId) && current.stock > 1 ) {
      localStorageValue = JSON.parse(localStorage.getItem(localStorageKey));
      substractItem = JSON.parse(localStorage.getItem(localStorageKey)).find(e => e.id === productId);
      substractItem.stock -= 1;
      localStorageValue = localStorageValue.filter(e => e.id !== productId);
      localStorageValue.push(substractItem);
      localStorage.setItem(localStorageKey, JSON.stringify(localStorageValue));
    }
    refreshInfoForModifyQuantity(productId, e.target);
  }
